...
for(var i=0; i<array_length(strg_label); i++){
	...
	switch obj_game_manager.plrstate{
		case 0:
			with instance_create_depth(obj_player.x+30,obj_player.y-i*24,-9999,obj_strg_item){
				sprite_index=_spr;
				label=_label;
			}
		break;
		case 1:
			with instance_create_depth(camera_get_view_x(view_camera[0])+40+i*32,camera_get_view_y(view_camera[0])+200,-9999,obj_strg_item){
				sprite_index=_spr;
				label=_label;
			}
		break;
